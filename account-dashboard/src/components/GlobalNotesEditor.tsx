...
interface GlobalNotesEditorProps {
  content: string;
  lastSaved: string | null;
  onSave: (content: string) => Promise<void>;
}

export function GlobalNotesEditor({ content, lastSaved, onSave }: GlobalNotesEditorProps) {
  const [saving, setSaving] = useState(false);
  const [saved, setSaved] = useState(false);
  const saveTimeout = useRef<ReturnType<typeof setTimeout>>();
  const { height, onResizeStart } = useResizableHeight({
    storageKey: 'globalNotesHeight',
    defaultHeight: 300,
  });
  ...
  const editor = useEditor({
    extensions: [
      ...
      Placeholder.configure({
        placeholder: 'Quick reference notes visible on all accounts... (auto-saves)',
      }),
    ],
    content: content || '',
    editorProps: {
  ...
  return (
    <div className="bg-dark-800/60 border border-amber-500/20 rounded-xl overflow-hidden">
      <div className="flex items-center justify-between px-4 py-2.5 border-b border-dark-700/50">
        <div className="flex items-center gap-2">
          <StickyNote className="w-4 h-4 text-amber-400" />
          <h2 className="text-sm font-semibold text-dark-200">Quick Reference Notes</h2>
